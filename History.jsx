    ...
    <div className={`main-container ${!isSidebarOpen ? 'sidebar-closed' : ''}`}>
      <div className={`layout-left ${!isSidebarOpen ? 'collapsed' : ''}`}>
        ...
        <nav>
           ...
           <Link to="/historique" className="history">
          ...
          <tbody>
            {history.map(a=>(
             <tr >
              ...
              <td>{nom}</td>
              <td>{prenom}</td>
              <td>{new Date(a.date_analyse).toLocaleString()}</td>
               <td>{a.NomMalade || 'Non disponible'}</td>
              <td>  <img 
                        src={`data:image/jpeg;base64,${a.chemin}`} 
                        alt="Résultat d'analyse"
                        style={{ width: '100px', height: 'auto' }}
                      /></td>
               <td>{a.taux}%</td>
               {a.medicaments.map((m,index)=>( <td key={index}>{m.nom}</td>))
             }
            </tr>
))}
          </tbody>
